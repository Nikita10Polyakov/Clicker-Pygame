...
'''клас прямокутник'''
max_points = int(input('Скільки очок потрібно для виграшу?'))
...
for i in range(4):
    ...
    new_card = Label(x, 370, 70, 100, YELLOW)
    new_card.outline(BLUE, 10)
    new_card.set_text('CLICK', 26)
    cards.append(new_card)
...
while True:
    ...
    if new_time - start_time >= 11 + max_points:
    ...
    if points >= max_points:
        print("Кількість ваших очок -", max_points * (new_time - start_time))
        if max_points * (new_time - start_time) < 30:
            print('Таблиця лідерів:\nSasha - 120\n', name, '-', round(max_points * (new_time - start_time), 3))
        else:
            print(('Таблиця лідерів:', name, '-', round(max_points * (new_time - start_time), 3)), '\nSasha - 120\n')
